<?php 
	...
	if(isset($_SESSION['id_usuario'])){ 
		...
		$result = $db->getRows("SELECT * FROM tbl_usuario WHERE id <> 1 ORDER BY id DESC");
		...
		if(isset($_GET['erro'])){
			...
			if($_GET['msg'] == "success"){
				$mensagem = '<div class="alert alert-success .alert-dismissable col-sm-12 col-md-12 col-lg-12">
								<a class="close" data-dismiss="alert" aria-label="close">&times;</a>Usuário cadastrado com <strong>Sucesso</strong>
							</div>';
			}else if($_GET['msg'] == "excluido"){
				$mensagem = '<div class="alert alert-success .alert-dismissable col-sm-12 col-md-12 col-lg-12">
								<a class="close" data-dismiss="alert" aria-label="close">&times;</a>Usuário excluído com <strong>Sucesso</strong>
							</div>';
			}else if($_GET['msg'] == "changed"){
				$mensagem = '<div class="alert alert-success .alert-dismissable col-sm-12 col-md-12 col-lg-12">
								<a class="close" data-dismiss="alert" aria-label="close">&times;</a>Usuário alterado com <strong>Sucesso</strong>
							</div>';
			}else {
				$mensagem = "";
			}
